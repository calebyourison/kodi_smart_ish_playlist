...
def gather_media_info(monitor:xbmc.Monitor) -> dict[str, dict]:
    ...
    for show in defined_show_criteria:
        ...
        exclusions:dict[int, str] = {item.get("id"): item.get("title") for item in show.get("exclusions", [])}
        ...
        all_show_episodes:list[dict] = list_of_episodes_by_show_id(show_id=show_id, number=(number_of_episodes + len(exclusions) * 2) )
        non_excluded_episodes:list[dict] = [item for item in all_show_episodes if item.get("episodeid") not in exclusions.keys()]

        if number_of_episodes >= len(non_excluded_episodes):
            selection:list[dict] = non_excluded_episodes

        else:
            selection:list[dict] = random.sample(non_excluded_episodes, number_of_episodes)
    ...
    if number_of_movies >= len(selected_movies):
        ...
        final_movie_selection:list[dict] = random.sample(selected_movies, number_of_movies)

    movies_dict: dict[int, str] = {item.get("id"): item.get("title") for item in final_movie_selection}
...
def playlist_builder(media_info:dict[str, dict], monitor:xbmc.Monitor, progress_queue:Queue, cancel_event:threading.Event, clear_existing:bool=True, playlist_id:int=1) -> bool:
